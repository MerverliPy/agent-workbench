...
| **Short-term** | 27 | 2–3 weeks | Remote access, session sharing, export/import |
...
[⬜] Works on iOS Safari + Android Chrome (minimum: last 2 versions each) — verified on iOS Safari
...
[⬜] All 523+ tests continue to pass — verify with `bun test`
[⬜] apps/mobile-web typecheck: tsc --noEmit passes clean — verify with `cd apps/mobile-web && bun run typecheck`
```
