...
export function GitTreePanel(): JSX.Element {
  ...
  async function loadGitInfo(): Promise<void> {
    ...
    try {
      ...
      if (
        ...
        setError(
          "Not a git repository. Run the server from a git project directory.",
        );
      } else {
        setError(
          err instanceof Error ? err.message : "Failed to load git info",
        );
      }
    } finally {
      setLoading(false);
  ...
  return (
    <div class="flex flex-col h-full panel-enter">
      ...
      <Show when={error()}>
        <div class="px-4 py-3">
          <div class="text-sm text-red-400 bg-slate-800/50 rounded-lg px-3 py-2">
            {error()}
          </div>
        </div>
      </Show>
        ...
        <div class="flex-1 overflow-y-auto px-4 py-3 space-y-4">
          ...
          <div class="bg-slate-800/50 rounded-lg p-3">
            ...
            <div class="flex gap-4 text-xs text-slate-400">
              <span>
                <span class="text-yellow-400">{data()?.dirtyFiles}</span>{" "}
                modified
              </span>
              <span>
