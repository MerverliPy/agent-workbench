...
function parseRepo(repo?: string): { owner: string; repo: string } | { error: string } {
  if (!repo) {
    return { error: "Repository is required. Format: owner/repo (e.g. 'MerverliPy/agent-workbench')" };
  }
...
export const tools = [
  // ── github.list_issues ─────────────────────────────────────────────────
  {
    ...
    description: "List issues for a GitHub repository. Can filter by state (open/closed) and labels.",
    parameters: {
      type: "object",
      properties: {
        repo: {
          type: "string",
          description: "Repository in owner/repo format (e.g. 'MerverliPy/agent-workbench')",
        },
        state: {
          type: "string",
    ...
    async execute(input: Record<string, unknown>): Promise<PluginToolResult> {
      ...
      try {
        ...
        if (!result.success) {
          return { content: result.error ?? "GitHub API error", success: false, error: result.error };
        }
        ...
        if (actualIssues.length === 0) {
          return { content: `No ${state} issues found for ${repo.owner}/${repo.repo}.`, success: true };
        }
        ...
        return { content, success: true, data: { count: actualIssues.length, issues: actualIssues } };
      } catch (err) {
  ...
  // ── github.search_code ─────────────────────────────────────────────────
  {
    ...
    description: "Search code across GitHub repositories. Uses GitHub's code search syntax (e.g. 'repo:owner/name query').",
    parameters: {
      type: "object",
      properties: {
        query: {
          type: "string",
          description: "Search query. Can include repo:owner/name, language:js, path:/src, etc.",
        },
        limit: {
          type: "number",
    ...
    async execute(input: Record<string, unknown>): Promise<PluginToolResult> {
      ...
      if (!query || query.trim().length === 0) {
        return { content: "Search query is required.", success: false, error: "Search query is required" };
      }
      ...
      try {
        ...
        const total = (result.data as Record<string, unknown>)?.["total_count"] ?? 0;

        if (items.length === 0) {
          return { content: `No code results found for "${query}".`, success: true };
        }

        const lines = items.map((item: Record<string, unknown>) => {
          const name = item["name"] as string;
          const path = item["path"] as string;
          const repoName = (item["repository"] as Record<string, unknown>)?.["full_name"] ?? "unknown";
  ...
  {
    name: "github.get_pr",
    description: "Get details of a specific pull request, including the diff summary.",
    parameters: {
      type: "object",
      properties: {
        repo: {
    ...
    async execute(input: Record<string, unknown>): Promise<PluginToolResult> {
      ...
      if (!prNumber || prNumber < 1) {
        return { content: "A valid PR number is required.", success: false, error: "Invalid PR number" };
      }
      ...
      try {
        ...
        if (!prResult.success) {
          return { content: prResult.error ?? "GitHub API error", success: false, error: prResult.error };
        }
  ...
  // ── github.create_issue ────────────────────────────────────────────────
  {
    ...
    description: "Create a new issue on a GitHub repository. Requires GITHUB_TOKEN to be set.",
    parameters: {
      type: "object",
      properties: {
        repo: {
        ...
        body: {
          type: "string",
          description: "Issue body / description (optional, markdown supported)",
        },
        labels: {
          type: "array",
          items: { type: "string" },
    ...
    async execute(input: Record<string, unknown>): Promise<PluginToolResult> {
      ...
      if (!token) {
        return {
          content: "GITHUB_TOKEN or GH_TOKEN environment variable is required to create issues.",
          success: false,
      ...
      if (!title || title.trim().length === 0) {
        return { content: "Issue title is required.", success: false, error: "Issue title is required" };
      }
  ...
  // ── github.list_repos ──────────────────────────────────────────────────
  {
    ...
    description: "List repositories for a user or organization. When authenticated, lists your repos; otherwise public repos for the given user.",
    parameters: {
      type: "object",
      properties: {
        owner: {
          type: "string",
          description: "User or organization name (e.g. 'MerverliPy'). Defaults to authenticated user if GITHUB_TOKEN is set.",
        },
        type: {
          type: "string",
          description: "Type of repos to list: 'all', 'owner', 'public', 'private', 'forks', 'sources', 'member' (default: 'owner')",
    ...
    async execute(input: Record<string, unknown>): Promise<PluginToolResult> {
      ...
      try {
        ...
        if (repos.length === 0) {
          ...
          return { content: `No repositories found for ${who}.`, success: true };
        }
        ...
        return { content: `Repositories:\n\n${lines.join("\n")}`, success: true, data: { count: repos.length, repos } };
      } catch (err) {
        const msg = `Network error calling GitHub API: ${err instanceof Error ? err.message : String(err)}`;
        return { content: msg, success: false, error: msg };
      }
