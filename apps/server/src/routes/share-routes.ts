...
export function registerShareRoutes(app: Hono<ServerAppBindings>, services: ServerServices): void {
  const { shareManager, sessionRepository, messageRepository, toolCallRepository } = services;
  ...
  app.post("/session/:sessionId/share", async (c) => {
    try {
      ...
      const body = (await c.req.json().catch(() => ({}))) as { label?: string; expiresInMs?: number };
      ...
      if (body.expiresInMs !== undefined) options.expiresInMs = body.expiresInMs;
      ...
      return handleAppError(
        err instanceof Error
          ? new ApiError({ status: 500, code: "SHARE_CREATE_FAILED", message: err.message, recoverable: false })
          : new ApiError({ status: 500, code: "SHARE_CREATE_FAILED", message: "Unknown error", recoverable: false }),
        c,
      );
    }
  });
  ...
  app.get("/share/:token", async (c) => {
    try {
      ...
      if (!session) {
        return c.json(
          new ApiError({ status: 404, code: "NOT_FOUND", message: "Session not found.", recoverable: true }),
          404,
        );
      }
      ...
      return handleAppError(
        err instanceof Error
          ? new ApiError({ status: 500, code: "SHARE_ACCESS_FAILED", message: err.message, recoverable: false })
          : new ApiError({ status: 500, code: "SHARE_ACCESS_FAILED", message: "Unknown error", recoverable: false }),
        c,
      );
    }
  });
  ...
  app.get("/session/:sessionId/shares", async (c) => {
    try {
      ...
      return handleAppError(
        err instanceof Error
          ? new ApiError({ status: 500, code: "SHARE_LIST_FAILED", message: err.message, recoverable: false })
          : new ApiError({ status: 500, code: "SHARE_LIST_FAILED", message: "Unknown error", recoverable: false }),
        c,
      );
    }
  });
  ...
  app.delete("/share/:token", async (c) => {
    try {
      ...
      if (!record) {
        return c.json(
          new ApiError({ status: 404, code: "SHARE_NOT_FOUND", message: "Share token not found.", recoverable: true }),
          404,
        );
      }

      shareManager.revoke(token);
      return c.json({ message: "Share revoked.", token, sessionId: record.sessionId });
    } catch (err) {
      return handleAppError(
        err instanceof Error
