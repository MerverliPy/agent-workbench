...
export function MessageBubble(props: MessageBubbleProps): JSX.Element {
  ...
  if (role === "system") {
    return (
      <div class="flex justify-center msg-in">
        <span class="text-xs italic px-3 py-1" style="color: var(--muted);">{content}</span>
      </div>
    );
  }
  ...
  if (cardType && cardData) {
    return (
      <div class="msg-in">
        <CardRegistry cardType={cardType as "plan" | "tool" | "diff" | "terminal" | "approval" | "summary"} cardData={cardData} />
      </div>
    );
  }
  ...
  return (
    <div class={"flex msg-in " + (isUser ? "justify-end" : "justify-start")}>
      <div
        class={"max-w-[85%] rounded-2xl px-3.5 py-2.5 " + (isUser ? "rounded-br-md" : "rounded-bl-md")}
        style={
          isUser
      ...
      >
        {isUser ? (
          ...
          <div class="text-sm markdown-body" innerHTML={renderMarkdown(content)} />
        )}
